...
export const MarkerContext2 = createContext();

export const MarkerProvider2 = ({ children }) => {
    // 시간 "HHMM" → "HH:MM"
    const fmtHM = (s) =>
    ...
    const buildOverlayHTML = (park, idx) => {
        ...
        return `
          <div class="ep-overlay ep-overlay2">
            <div class="ep-overlay__head">
              <div class="ep-overlay__title ep-overlay__title2">${
            park.PKLT_NM || "주차장"
        }</div>
              <div class="ep-overlay__badge ep-overlay__badge2">${park.CHGD_FREE_NM || ""}</div>
              <button class="ep-close ep-close2" aria-label="닫기">닫기</button>
            </div>
            <div class="ep-overlay__body ep-overlay__body2">
              <div class="ep-kv" >
                <span class="ep-kv2">총자리</span><b>${total}</b>
                <span class="ep-kv2">현재</span><b>${live}</b>
                <span class="ep-kv2">남음</span><b>${remain}</b>
              </div>
              <div class="ep-row ep-row2"><span>가격 (5분당)</span><b>${price}</b></div>
              <div class="ep-row ep-row2"><span>운영시간</span><b>${wd}</b></div>
              <div class="ep-row ep-row2"><span>유형</span><b>${
            park.PKLT_KND_NM || "-"
        }</b></div>
    ...
    return (
        <MarkerContext2.Provider value={{buildMarkerImage,buildOverlayHTML}}>
            {children}
        </MarkerContext2.Provider>
    );
};
